...
// как удар (переключений не меньше STRIKE_MIN_EDGES), либо отбрасывается. Итог каждой пачки
// пишется в журнал и уходит в приложение, чтобы подбирать порог под конкретный посох.
...
bool strikeDetected() {
  ...
  logLine(LOG_STRIKE, LOG_INFO, "%d переключений за %lu мс, %s", edges, STRIKE_WINDOW_MS,
